...
// missing prototypes
VolumeDescriptor_t *VolumeInfo(DeviceInfo_t *devp);
int CompareVolumeHeaders(HFSPlusVolumeHeader *left, HFSPlusVolumeHeader *right);
int AddHeaders(VolumeObjects_t *vop);
void AddJournal(VolumeObjects_t *vop);
void AddFileExtents(VolumeObjects_t *vop);
void DumpData(const void *data, size_t len);
int doCleanup(struct IOWrapper *ctx);
struct IOWrapper * InitDeviceWrapper(const char *path, DeviceInfo_t *devp);
